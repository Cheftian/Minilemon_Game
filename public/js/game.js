// ===== 1. Global Variables =====
  ...
  async function fetchRoomsPosition(spacesId) {
    ...
    if (arrowKeys.includes(key)) {
      ...
      switch (key) {
        case 'ArrowUp':
          mapWrapper.scrollBy({ top: -PAN_SPEED, behavior: 'smooth' });
          break;
        case 'ArrowDown':
          mapWrapper.scrollBy({ top: PAN_SPEED, behavior: 'smooth' });
          break;
        case 'ArrowLeft':
          mapWrapper.scrollBy({ left: -PAN_SPEED, behavior: 'smooth' });
          break;
        case 'ArrowRight':
          mapWrapper.scrollBy({ left: PAN_SPEED, behavior: 'smooth' });
          break;
    ...
    // Logika gerakan pemain tetap sama
    if (isMoving) return;
    ...
    async function checkAreas(x, y, areas, label) {
      ...
      for (const area of areas) {
        if (x < area.x + area.width && x + 32 > area.x && y < area.y + area.height && y + 32 > area.y) {
          foundArea = area;
          break;
        }
      }
      ...
      if (foundArea && foundArea.id !== currentAreaObjectId) {
        ...
        try {
          const areaDataResponse = await fetch(`http://localhost:8000/chat_areas/object/${foundArea.id}`);
          const areaData = await areaDataResponse.json();
          const chatAreaIdFromServer = areaData.ChatArea_ID;

          if (chatAreaIdFromServer) {
            activeChatAreaId = chatAreaIdFromServer;
            currentAreaObjectId = foundArea.id;
            isInArea = true;

            await fetch(`http://localhost:8000/userposition/${spacesMemberId}/enterchat/${activeChatAreaId}`, {
                method: "PUT",
                headers: { 'Authorization': `Bearer ${token}` }
            });
            console.log(`[API] Entered chat area: ${activeChatAreaId}`);

            const chatResponse = await fetch(`http://localhost:8000/chats/chatarea/${activeChatAreaId}`);
            const chatData = await chatResponse.json();
            
            if (chatData && chatData.length > 0) {
              areaChatId = chatData[0].Chats_ID; 

              const memberRes = await fetch(`http://localhost:8000/chat_members/${areaChatId}/${userId}`);
              const memberData = await memberRes.json();
              areaChatMemberId = memberData.ChatsMember_ID;

              // Beri tahu server WebSocket bahwa kita sekarang mendengarkan di saluran Area
              sendWebSocketRequest("join_chat", { Chats_ID: areaChatId });
            
              if (currentMode === "Area") {
                input.disabled = false;
                chatModeText.textContent = `Area - ${foundArea.name}`;
                await loadChatHistory(areaChatId);
              }
            }
          }
        } catch (err) {
          console.error(`Gagal memproses ${label}:`, err);
        }

      } else if (!foundArea && isInArea) {
        await leaveChatArea();
        isInArea = false;
      }
    }
    ...
    async function leaveChatArea() {
      if (!activeChatAreaId) return;

      try {
        await fetch(`http://localhost:8000/userposition/${spacesMemberId}/leavechat`, {
            method: "PUT",
            headers: { 'Authorization': `Bearer ${token}` }
        });
        console.log(`[API] Left chat area: ${activeChatAreaId}`);
        
        // Beri tahu server WebSocket bahwa kita kembali ke saluran Broadcast
        sendWebSocketRequest("join_chat", { Chats_ID: chatIdBroadcast });
        
        activeChatAreaId = null;
        currentAreaObjectId = null;
        areaChatId = null;
        areaChatMemberId = null;

        if (currentMode === "Area") {
          chatModeText.textContent = `Area`;
          displayAreaPlaceholder();
        }

      } catch (err) {
        console.error("Gagal saat keluar dari chat area:", err);
      }
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      console.log("[DEBUG] Chat Input:", input);
    ...
    mapWrapper.addEventListener('wheel', (e) => {
      // Cek jika scroll terjadi di dalam map-wrapper, BUKAN di panel
      if (e.target.closest('#map-container-wrapper') && !e.target.closest('.panel')) {
        e.preventDefault(); // Mencegah scroll default halaman
        
        const delta = Math.sign(e.deltaY);
        const zoomStep = 0.1;

        if (delta < 0 && zoomLevel < 2) { // Zoom In
          zoomLevel += zoomStep;
        } else if (delta > 0 && zoomLevel > 1) { // Zoom Out
          zoomLevel -= zoomStep;
        }
        
        // Terapkan transform HANYA ke mapWrapper
    ...
    function stopMapZoom(event) {
      event.stopPropagation();
    }

    chatPanel.addEventListener('wheel', stopMapZoom);
    participantsPanel.addEventListener('wheel', stopMapZoom);
  ...
  let areaChatId = null;
  let areaChatMemberId = null;

  // Fungsi untuk menampilkan pesan "Masuk ke area..."
  function displayAreaPlaceholder() {
    messages.innerHTML = `<div class="chat-placeholder">Masuk ke area obrolan untuk memulai obrolan kelompok.</div>`;
    input.disabled = true;
  }
  ...
  function renderChatMessage(messageData) {
    ...
    profilePic.className = "chat-info";
    const img = document.createElement("img");
    const selfImage = document.getElementById("footer-profile-img").src;
    img.src = isSelf ? selfImage : (Profile_Image || "https://via.placeholder.com/32");
    ...
    if (!isSelf) {
        ...
        bubbleWrapper.appendChild(name);
    }
    ...
    timestamp.className = "chat-timestamp-bubble";
    if (created_at) {
    ...
    // Masukkan teks dan timestamp KE DALAM BUBBLE
  ...
  // Logika saat mengganti mode chat
  ...
  modeButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (currentMode === "Broadcast") {
        chatModeText.textContent = `Broadcast - ${space.name}`;
        sendWebSocketRequest("join_chat", { Chats_ID: chatIdBroadcast });
        loadChatHistory(chatIdBroadcast);
      } else if (currentMode === "Area") {
        if (activeChatAreaId && areaChatId) {
          const areaName = chatAreas.find(area => area.id === currentAreaObjectId)?.name || 'Unknown Area';
          chatModeText.textContent = `Area - ${areaName}`;
          sendWebSocketRequest("join_chat", { Chats_ID: areaChatId });
          loadChatHistory(areaChatId);
        } else {
          chatModeText.textContent = `Area`;
          displayAreaPlaceholder();
        }
  ...
  // Fungsi terpusat untuk mengirim pesan
  async function sendMessage() {
    const msg = input.value.trim();
    if (!msg) return; // Jangan kirim jika kosong
    ...
    // Tampilkan pesan sendiri secara optimis
    renderChatMessage({
    ...
    let targetChatMemberId = null;

    if (currentMode === "Broadcast") {
        targetChatId = chatIdBroadcast;
        targetChatMemberId = chatMemberId;
    } else if (currentMode === "Area" && areaChatId) {
        targetChatId = areaChatId;
        targetChatMemberId = areaChatMemberId;
    }
    
    if (targetChatId && targetChatMemberId) {
        sendWebSocketRequest("send_chat_message", {
          SpacesMember_ID: spacesMemberId,
          Chats_ID: targetChatId,
          ChatsMember_ID: targetChatMemberId,
          Message: msg,
          Username: username,
